...
const App: React.FC = () => {
  ...
  const navigateTo = (view: 'landing' | 'work' | 'admin') => {
      ...
      // Optional: window.location.reload() would force the PWA manifest script in index.html to re-run,
      // but usually we want to keep the SPA state.
  };
        ...
        <header className={`p-6 border-b border-gray-800/50 backdrop-blur-md ${isAdminMode ? 'bg-red-900/40 border-red-500/30' : 'bg-brand-black/80'}`}>
            <div className="flex justify-between items-center mb-6">
                <div 
                    onClick={() => {
                        if(isAdminMode){
                            navigateTo('work'); // In Admin view, logo click goes to WORK as requested
            ...
            <div className="space-y-6">
                ...
                {deferredPrompt && (
                   <div className="p-6 bg-blue-600/10 border border-blue-600/20 rounded-[45px] flex flex-col items-center gap-4 shadow-2xl">
                      ...
                      <Button onClick={handleInstallClick} className="w-full py-5 rounded-[35px] bg-blue-600 text-white font-black uppercase text-xs shadow-3xl shadow-blue-600/40 hover:scale-105 active:scale-95 transition-all">📲 הורד עכשיו</Button>
                   </div>
                )}
