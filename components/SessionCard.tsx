...
export const SessionCard: React.FC<SessionCardProps> = ({ 
  ...
  return (
    <div 
      ...
      className={`relative rounded-2xl p-3 border shadow-lg cursor-pointer transition-all hover:-translate-y-1 group flex flex-col gap-2 h-full justify-between ${containerClass}`}
    ...
    >
      <div className="absolute top-0 left-0 flex flex-row gap-1 p-1 z-10 flex-wrap max-w-full">
        {isCancelled ? (
           <div className="bg-red-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full shadow-lg">מבוטל ✕</div>
        ) : (
            <>
                {isZoomOnly && <div className="bg-blue-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full shadow-lg">🎥 זום בלבד</div>}
                {isHybrid && <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full shadow-lg">🌳+🎥 היברידי</div>}
                {session.isHidden && <div className="bg-purple-900 text-white text-[9px] font-black px-2 py-0.5 rounded-full">👻 נסתר</div>}
            </>
        )}
      </div>
      ...
      <div className="flex justify-between items-start mt-5">
        ...
        {displayIcon && !isCancelled && (
            <div className="flex items-center gap-1 text-[9px] text-gray-400 bg-gray-800/50 px-2 py-1 rounded-full">
      ...
      <div>
          <h3 className={`font-black text-sm leading-tight mb-2 line-clamp-1 ${isCancelled ? 'text-gray-500' : ''}`} style={{ color: isCancelled ? undefined : cardColor }}>{session.type}</h3>
          <div 
            className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[9px] font-bold border w-full truncate ${defaultLocationClass} ${isCancelled ? 'opacity-50' : ''}`}
            style={locationBadgeStyle && !isCancelled ? locationBadgeStyle : {}}
          >
            <span>{isZoomOnly ? '💻 זום אונליין' : session.location}</span>
          </div>
      </div>

      <div className="mt-auto pt-2 border-t border-gray-800/50 flex flex-col gap-2">
          <div className="flex justify-between items-center text-[9px] font-bold">
             <span className={`${isFull && !isCancelled ? 'text-red-400' : 'text-gray-300'}`}>{registeredCount}/{session.maxCapacity} רשומים</span>
          </div>
          <Button 
            size="sm" 
            variant={isAdmin ? 'danger' : isCancelled ? 'secondary' : isRegistered ? 'outline' : 'primary'}
            className="w-full text-[10px] py-1 h-7 font-black"
            onClick={(e)=>{e.stopPropagation(); if(!isCancelled || isAdmin) onRegisterClick(session.id);}}
